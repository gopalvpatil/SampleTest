 ...
 */
@Component
public class DailyStopFileJobServiceImpl implements DailyStopFileJobService {
    @Value("${dailystop.file.job.location}")
    public String stopFileJobLocation;
    ...
    @Override
    public Map<String, Integer> pullStopFile() throws NASConnectException {
        ...
        try {
	        File dailyStopFileDirectory = new File(stopFileJobLocation);
	        filesList = dailyStopFileDirectory.listFiles();
        } catch (Exception e) {
        ...
        for (File file : filesList) {
            if (!file.isDirectory()) {
                ...
                logger.info(Log.event(Event.DAILY_STOP_FILE_FILE_READ_SUCCESS, "Successfully read Daily Stop file " + file.getName() + " located at " + file.getPath()));
            }
        }
        ...
        for (PositivePayFtpFile positivePayFtpFile : positivePayFtpFileList) {
            try {
                byte[] bytesRead = fileUploadUtils.readUtfBytes(positivePayFtpFile.getPath());
