 ...
 */
@Service
public class IssuedAfterVoidServiceImpl implements IssuedAfterVoidService {
    @Loggable
    Logger logger;
    ...
    @Override
    public Map<String, Integer> markChecksIssuedAfterVoid() {
        ...
        //List of all checks from check detail table which are in void status where check number,
        ...
        for (ExceptionCheckDto expCheckDto : ExpCheckDtos) {
        	...
        	ExceptionalCheck expCheckToDelete = expCheckDao.findById(expCheckDto.getExceptionCheckId());
                ...
                workflowService.performAction(expCheckDto.getId(), "issuedAfterVoid", userData);
