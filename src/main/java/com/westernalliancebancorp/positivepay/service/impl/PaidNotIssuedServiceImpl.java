 ...
 */
@Service
public class PaidNotIssuedServiceImpl implements PaidNotIssuedService {
    ...
    @Override
    public Map<String, Integer> markChecksPaidNotIssuedByAccounts(List<Account> accounts) {
        List<Long> accountIds = Lambda.extract(accounts, on(Account.class).getId());
        return markChecksPaidNotIssuedByAccountIds(accountIds);
    }

    @Override
    public Map<String, Integer> markChecksPaidNotIssuedByCompany(List<Company> company) {
        List<Long> companyIds = Lambda.extract(company, on(Company.class).getId());
        return markChecksPaidNotIssuedByCompanyIds(companyIds);
    }

    @Override
    public Map<String, Integer> markCheckPaidNotIssuedByBank(List<Bank> banks) {
        List<Long> bankIds = Lambda.extract(banks, on(Bank.class).getId());
        return markChecksPaidNotIssuedByBankIds(bankIds);
    }

    @Override
    public Map<String, Integer> markChecksPaidNotIssuedByCompanyIds(List<Long> companyIds) {
        List<Account> accountsList = accountDao.findAllByCompanyIds(companyIds);
        return markChecksPaidNotIssuedByAccounts(accountsList);
    }

    @Override
    public Map<String, Integer> markChecksPaidNotIssuedByBankIds(List<Long> bankIds) {
    ...
    public Map<String, Integer> markChecksPaidNotIssuedByAccountIds(List<Long> accountIds) {
        Map<String, Integer> returnMap = new HashMap<String, Integer>();
        int itemsProcessedSuccessfuly = 0;
        int itemsInError = 0;
        List<Long> referenceDataIds = batchDao.findAllPaidNotIssuedChecks(accountIds);
            ...
            if (!referenceDatas.isEmpty()) {
                for (ReferenceData referenceData : referenceDatas) {
                	...
                	logger.debug("Delegating to Reference Data Processor Service : Reference Data Id"+referenceData.getId());
                        ...
                        logger.error(Log.event(Event.MARK_PAIDNOTISSUED_UNSUCCESSFUL, e.getMessage() +" ReferenceData Id "+referenceData.getId(),e), e);
                        itemsInError++;
