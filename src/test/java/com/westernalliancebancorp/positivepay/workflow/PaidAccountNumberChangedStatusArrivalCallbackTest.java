...
/**
 *  User: moumita 
 *  Date: 20/4/14 Time: 4:46 PM
 */
@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration(locations = { "classpath*:positivepay-test-context.xml" })
public class PaidAccountNumberChangedStatusArrivalCallbackTest {
	@Autowired
	...
	@Test
	public void testPaidAccountNumberChangedStatusArrivalCallback()
			throws CallbackException, WorkFlowServiceException {
		...
		check.setCheckNumber(PaidAccountNumberChangedStatusArrivalCallbackTest.shortUUID());
		...
		userData.put(
				WorkflowService.STANDARD_MAP_KEYS.ACCOUNT_NUMBER_NEW.name(),
				"3105978603098422040");
		CheckDto checkDto = new CheckDto();
		checkDto.setReferenceDataId(createReferenceData(check).getId());
		userData.put(Constants.CHECK_DTO, checkDto);
		
		String series = "created,matched,changeAccountNumber";
		for (String actionToPerform : series.split(",")) {
			String result = workflowService.performAction(check.getId(),
					actionToPerform, userData);
