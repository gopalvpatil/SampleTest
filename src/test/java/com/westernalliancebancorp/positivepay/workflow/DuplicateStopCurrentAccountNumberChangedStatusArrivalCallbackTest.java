...
public class DuplicateStopCurrentAccountNumberChangedStatusArrivalCallbackTest {
    @Autowired
    ...
    public void testDuplicateStopCurrentAccountNumberChanged() throws CallbackException, WorkFlowServiceException {
        ...
        check.setCheckNumber(DuplicateStopCurrentAccountNumberChangedStatusArrivalCallbackTest.shortUUID());
        ...
        checkDto.setReferenceDataId(createReferenceData(check).getId());
        userData.put(Constants.CHECK_DTO, checkDto);
        series = "duplicateStop,changeCurrentAccountNumber";
        for (String actionToPerform : series.split(",")) {
            if (actionToPerform.equals("changeCurrentAccountNumber")) {
