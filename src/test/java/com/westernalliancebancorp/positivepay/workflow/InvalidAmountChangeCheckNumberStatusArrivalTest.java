...
public class InvalidAmountChangeCheckNumberStatusArrivalTest {
    @Autowired
    ...
    @Test
    public void testInvalidAmountChangeCheckNumberStatusArrivalCallBack() throws CallbackException, WorkFlowServiceException {
        ...
        check.setCheckNumber(InvalidAmountChangeCheckNumberStatusArrivalTest.shortUUID());
        ...
        // -> changeCheckNumber(a) -> InvalidAmountCheckNumberChanged(s) (InvalidAmountCheckNumberChangedStatusArrival will trigger and move it back to issued)
        String series = "created,changeCheckNumber";
    ...
    @Test
    public void testInvalidAmountChangeCheckNumberStatusArrivalExistingCheckCallBack() throws CallbackException, WorkFlowServiceException {
        Account account = accountDao.findById(1l);
        ItemType itemType = itemTypeDao.findByCode(ItemType.CODE.I.name());
        CheckStatus checkStatus = checkStatusDao.findByNameAndVersion("start", 1);
        Check check = new Check();
        ...
        for (String actionToPerform : series.split(",")) {
            if(actionToPerform.equals("changeCheckNumber")){
                String checkNumber = System.currentTimeMillis()+"";
                addCheck(checkNumber, check.getAccount());
                userData = new HashMap<String, Object>();
                userData.put(WorkflowService.STANDARD_MAP_KEYS.CHECK_NUMBER_NEW.name(), checkNumber);
            }
            else{
                userData = new HashMap<String, Object>();
            }
    ...
    @Test
    public void testInvalidAmountChangeCheckNumberAndChangedCheckNumberExisting()  throws CallbackException, WorkFlowServiceException {
        Account account = accountDao.findById(1l);
        ItemType itemType = itemTypeDao.findByCode(ItemType.CODE.I.name());
        CheckStatus checkStatus = checkStatusDao.findByNameAndVersion("start", 1);
        Check check = new Check();
        ...
        //created(a) -> Issued(s) -> invalidAmount(a) -> InvalidAmount(s) -> changeCheckNumber(a)
        // -> Invalid Amount Check number changed (a) -> InvalidAmountCheckNumberChangedStatusArrival
        String series = "created,invalidAmount,changeCheckNumber";
        for (String actionToPerform : series.split(",")) {
            String result = workflowService.performAction(check.getId(), actionToPerform, userData);
            System.out.println("Result is " + result);
        }
    ...
    @Test
    public void testInvalidAmountChangeCheckNumberAndChangedCheckNumberNotExisting()  throws CallbackException, WorkFlowServiceException {
        Account account = accountDao.findById(1l);
        ItemType itemType = itemTypeDao.findByCode(ItemType.CODE.I.name());
        CheckStatus checkStatus = checkStatusDao.findByNameAndVersion("start", 1);
        Check check = new Check();
        ...
        Map<String, Object> userData = new HashMap<String, Object>();
        userData.put(WorkflowService.STANDARD_MAP_KEYS.CHECK_NUMBER_NEW.name(), System.currentTimeMillis()+"");
        CheckDto checkDto = new CheckDto();
        checkDto.setReferenceDataId(createReferenceData(check).getId());
        userData.put(Constants.CHECK_DTO, checkDto);
        //created(a) -> Issued(s) -> invalidAmount(a) -> InvalidAmount(s) -> changeCheckNumber(a)
