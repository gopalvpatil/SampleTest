...
public class PaidNotIssuedAccountNumberChangedStatusArrivalCallbackTest {
    @Autowired
    ...
    @Test
    public void testPaidNotIssuedAccountNumberChanged() throws CallbackException, WorkFlowServiceException {
        //Insert the new check and reference Id with paid item code, issue PaidNotIssued command by putting the reference into the user data.
        //Take "ChangeCheckNumberAction"
        ...
        check.setCheckNumber(PaidNotIssuedAccountNumberChangedStatusArrivalCallbackTest.shortUUID());
        ...
        ReferenceData paidReferenceData = createReferenceData(check, ReferenceData.ITEM_TYPE.PAID);
        Map<String, Object> userData  = new HashMap<String, Object>();
        CheckDto checkDto = new CheckDto();
        checkDto.setReferenceDataId(paidReferenceData.getId());
        userData.put(WorkflowService.STANDARD_MAP_KEYS.CHECK_DTO.name(), checkDto);
        //Check will end up in paid not issued status after taking this workflow action.
        workflowService.performAction(check.getId(), "paidNotIssued", userData);
        //Take the action change account number.. it will make "StopNotIssuedStatusArrivalCallback" will be invoked.
        String series = "changeAccountNumber";
    ...
    @Test
    public void testPaidNotIssuedNewCheckIsNotNull() throws CallbackException, WorkFlowServiceException {
        ...
        //Take "ChangeAccountNumberAction"
        Account account = accountDao.findById(1l);
        ItemType itemType = itemTypeDao.findByCode(ItemType.CODE.I.name());
        Workflow workflow = workflowManagerFactory.getLatestWorkflow();
        CheckStatus checkStatus = ModelUtils.retrieveOrCreateCheckStatus(workflowManagerFactory.getWorkflowManagerById(workflow.getId()), "start", checkStatusDao);
        ...
        newCheck.setAccount(accountDao.findById(2l));
        newCheck.setCheckStatus(checkStatus);
        newCheck.setIssuedAmount(BigDecimal.valueOf(1000));
        newCheck.setCheckNumber(check.getCheckNumber());
        ...
        userData.put(WorkflowService.STANDARD_MAP_KEYS.REFERENCE_DATA.name(), paidReferenceData);
