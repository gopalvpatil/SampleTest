...
public class DuplicatePaidAccountNumberChangedStatusArrivalCallbackTest {
    @Autowired
    ...
    @Test
    @Ignore
    //No longer a valid scenario.
    public void testDuplicatePaidAccountNumberChangedStatusArrivalCallback() throws CallbackException, WorkFlowServiceException {
        ...
        check.setCheckNumber(DuplicatePaidAccountNumberChangedStatusArrivalCallbackTest.shortUUID());
        ...
        checkDto.setReferenceDataId(createReferenceData(check).getId());
        userData.put(Constants.CHECK_DTO, checkDto);
        series = "duplicatePaid,changeAccountNumber";
        for (String actionToPerform : series.split(",")) {
            String result = workflowService.performAction(check.getId(), actionToPerform, userData);
            System.out.println("Result is " + result);
        }
    ...
    @Test
    @Ignore
    //No loger a valid scenario. Reference data cannot be duplicated.
    public void testDuplicatePaidAccountNumberChangedStatusArrivalCallbackChangedAccountNumberExisting() throws CallbackException, WorkFlowServiceException {
        Account account = accountDao.findById(1l);
        ItemType itemType = itemTypeDao.findByCode(ItemType.CODE.I.name());
        CheckStatus checkStatus = checkStatusDao.findByNameAndVersion("start", 1);
        Check check = new Check();
